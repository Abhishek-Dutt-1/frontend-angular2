import {Post} from './post';

export var POSTS: Post[] = [
    {"id": 11, 
    "title": "Bryan Cranston", 
    "text": "Bryan Cranston played the role of Walter in Breaking Bad. He is also known for playing Hal in Malcom in the Middle.", 
    "type": "text",
    "comments": [{
      "id": 12,
      "title": "Comment title 12",
      "text": "Comment text 12",
      "comments": [{
        "id": 13,
        "title": "Reply Comment title 13",
        "text": "Reply Comment text 13",
        "comments": [{
          "id": 14,
          "title": "Reply Comment title 14",
          "text": "Reply Comment text 14"  
        }]  
      }]
    },
    {
      "id": 15,
      "title": "Comment title 15",
      "text": "Comment text 15",
      "comments": [{
        "id": 21,
        "title": "Comment title 21",
        "text": "Comment text 21",
        "comments": [{
          "id": 22,
          "title": "Reply Comment title 22",
          "text": "Reply Comment text 22",
          "comments": [{
            "id": 23,
            "title": "Reply Comment title 23",
            "text": "Reply Comment text 23"  
          }]  
        }]
      }]        
    },
    {
      "id": 16,
      "title": "Comment title 16",
      "text": "Comment text 16",
      "comments": []        
    },
    {
      "id": 17,
      "title": "Comment title 17",
      "text": "Comment text 17",
      "comments": []        
      },
    {
      "id": 18,
      "title": "Comment title 18",
      "text": "Comment text 18",
      "comments": []        
    },
    {
      "id": 19,
      "title": "Comment title 19",
      "text": "Comment text 19",
      "comments": [{
        "id": 21,
        "title": "Comment title 21",
        "text": "Comment text 21",
        "comments": []
      }]
    },
    {
      "id": 20,
      "title": "Comment title 20",
      "text": "Comment text 20",
      "comments": []        
    },
    {
      "id": 20,
      "title": "Comment title 20",
      "text": "Comment text 20",
      "comments": []        
    },
    {
      "id": 20,
      "title": "Comment title 20",
      "text": "Comment text 20",
      "comments": []        
    },
    {
      "id": 20,
      "title": "Comment title 20",
      "text": "Comment text 20",
      "comments": []        
    },
    {
      "id": 20,
      "title": "Comment title 20",
      "text": "Comment text 20",
      "comments": []        
    },
    {
      "id": 20,
      "title": "Comment title 20",
      "text": "Comment text 20",
      "comments": []        
    },
    {
      "id": 20,
      "title": "Comment title 20",
      "text": "Comment text 20",
      "comments": []        
    },
    {
      "id": 20,
      "title": "Comment title 20",
      "text": "Comment text 20",
      "comments": []        
    },
    {
      "id": 20,
      "title": "Comment title 20",
      "text": "Comment text 20",
      "comments": []        
    },
    {
      "id": 20,
      "title": "Comment title 20",
      "text": "Comment text 20",
      "comments": []        
    },
    {
      "id": 20,
      "title": "Comment title 20",
      "text": "Comment text 20",
      "comments": []        
    },
    {
      "id": 20,
      "title": "Comment title 20",
      "text": "Comment text 20",
      "comments": []        
    },
    {
      "id": 20,
      "title": "Comment title 20",
      "text": "Comment text 20",
      "comments": []        
    },
    {
      "id": 20,
      "title": "Comment title 20",
      "text": "Comment text 20",
      "comments": []        
    },
    {
      "id": 20,
      "title": "Comment title 20",
      "text": "Comment text 20",
      "comments": []        
    },
    {
      "id": 20,
      "title": "Comment title 20",
      "text": "Comment text 20",
      "comments": []        
    },
    {
      "id": 20,
      "title": "Comment title 20",
      "text": "Comment text 20",
      "comments": []        
    },
    {
      "id": 20,
      "title": "Comment title 20",
      "text": "Comment text 20",
      "comments": []        
    },
    {
      "id": 20,
      "title": "Comment title 20",
      "text": "Comment text 20",
      "comments": []        
    },
    {
      "id": 20,
      "title": "Comment title 20",
      "text": "Comment text 20",
      "comments": []        
    }
    ]},
    {"id": 12, 
    "title": "Aaron Paul", 
    "text": " Aaron Paul played the role of Jesse in Breaking Bad. He also featured in the \"Need For Speed\" Movie.", 
    type: "text",
    comments: []
    },
    {"id": 13, 
    "title": "Bob Odinkrik", 
    "text": "Bob Odinkrik played the role of Saul in Breaking Bad. Due to public fondness for the character, Bob stars in his own show now, called \"Better Call Saul\".", 
    type: "text",
    comments: []},
    {"id": 14, 
    "title": "Title 14", 
    "text": "Text 14", 
    type: "text",
    comments: []},
    {"id": 15, 
    "title": "Title 15", 
    "text": "Text 15", 
    type: "text",
    comments: []},
    {"id": 16, 
    "title": "Title 16", 
    "text": "Text 16", 
    type: "text",
    comments: []},
    {"id": 17, 
    "title": "Title 17", 
    "text": "Text 17", 
    type: "text",
    comments: []},
    {"id": 18, 
    "title": "Title 18", 
    "text": "Text 18", 
    type: "text",
    comments: []},
    {"id": 19, 
    "title": "Title 19", 
    "text": "Text 19", 
    type: "text",
    comments: []},
    {"id": 20, 
    "title": "TItle 20", 
    "text": "Text 20", 
    type: "text",
    comments: []}
];
